...
module.exports = function(restClient, bluebird) {
	/**
	...
	_.extend(Connector.prototype, {
		...
		executeActions: function (actions) {
			...
			_.each(targets, function(targetActions, targetUrl) {
				console.log('POSTing actions to: %s', targetUrl);
		...
		configureEventSource: function(eventSourceTemplate, eventSource) {
			...
			if (eventSourceTemplate.get('configurationUrl')) {
				console.log('Configuring event source at %s', eventSourceTemplate.get('configurationUrl'));
				...
				deferred.resolve();
				console.log('There is nothing to configure. Configuration URL is missing.');
			}
		...
		configureActionTarget: function(actionTargetTemplate, actionTarget) {
			...
			if (actionTargetTemplate.get('configurationUrl')) {
				console.log('Configuring action target at %s', actionTargetTemplate.get('configurationUrl'));
...
module.exports['@require'] = [ 'node-rest-client', 'bluebird' ];